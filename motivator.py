...
# === ТОКЕНЫ И НАСТРОЙКИ (БЕРУТСЯ ИЗ ПЕРЕМЕННЫХ ОКРУЖЕНИЯ RENDER) ===
...
def generate_prime_card(days_clean, saved_money, avoided_cigs, sos_count):
    ...
    try:
        # Убедись, что файл шрифта лежит в папке с ботом на GitHub
...
def ask_gemini(prompt_text, is_vision=False, image_data=None, system_context=""):
    base_instruction = (
        ...
        "Стиль: жесткий, прямолинейный, без воды. Никаких звездочек (*)."
    )
...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, "Бот запущен. Время МСК активно.", reply_markup=main_keyboard())
...
def handle_text(message):
    ...
    if text == '🆘 ХОЧУ КУРИТЬ (SOS)':
        ...
        prompt = "У меня тяга. Дай ОДНО короткое задание на 1 минуту (физика/дыхание) и напомни жестко, как курение убивает заживление акне."
        ...
        # Расчет дней от QUIT_DATE до текущего времени по МСК
        diff = get_now() - QUIT_DATE
        days_clean = diff.days
        ...
        msg = bot.send_message(chat_id, "Отправь состав текстом:", reply_markup=markup)
        bot.register_next_step_handler(msg, process_ingredient_check)

    elif text == '🧠 ИИ-Ассистент':
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True).add(types.KeyboardButton('❌ Отмена'))
        msg = bot.send_message(chat_id, "Задай вопрос или пришли фото:", reply_markup=markup)
        ...
        bot.send_message(chat_id, "Таймеры ухода:", reply_markup=markup)
...
def process_ingredient_check(message):
    if message.text == '❌ Отмена':
        bot.send_message(message.chat.id, "Отменено.", reply_markup=main_keyboard()); return
    bot.send_chat_action(message.chat.id, 'typing')
    bot.send_message(message.chat.id, ask_gemini(f"Проверь состав на комедогенность: {message.text}"), reply_markup=main_keyboard())
...
def handle_photo(message):
    ...
    bot.send_message(message.chat.id, ask_gemini("Оцени фото лица/состава.", is_vision=True, image_data=downloaded_file))
